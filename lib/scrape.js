const { keyAi, urlD } = require('../config');
...
function loli() {
  ...
  function komikusearch(text, apikey) {
    return new Promise(async(resolve, reject) => {
		...
		arr.push({
            ...
            Detail: `https://${urlD}/api/komikdetail?url=` + $(el).find('div:nth-child(2) > a ').attr('href').replace('https://komiku.id/manga/', '') + '&apikey='+ apikey,
		})
	})
	arr.map(item => {
...
function komikdetail(url) {
	return new Promise(async (resolve, reject) => {
	  try {
		const BASE_URL = `https://komiku.id/manga/${url}`;
		let html = (await axios.get(BASE_URL)).data;
		let $ = cheerio.load(html);
		let arr = [];
		let ch = [];
		$('article > section:nth-child(6) > div:nth-child(8) > table > tbody > tr').each((idx, el) => {
			let chapter = {
			  ...
			  link: 'https://komiku.id' + $(el).find('td > a').attr("href"),
			  date: $(el).find('td:nth-child(2)').text().trim(),
			};
			if (chapter.link && chapter.link !== "https://komiku.idundefined") {
			  ch.push(chapter);
			}
		  });
	await $('article').each((idx, el) => {
		  let data = {
			sinopsis: $(el).find('header > p').text().replace(/\n|\t/g, "").trim(),
			genre: $(el).find('section:nth-child(2) > ul > li > a').map((_, el) => $(el).text()).get().join(', '),
...
async function facebook(url) {
	return new Promise(async( resolve, reject) => {
        ...
        let arr = [];
        arr.push({
          sd: $('div.results-download > ul > li:nth-child(1) > a').attr('href'),
          hd: $('div.results-download > ul > li:nth-child(2) > a').attr('href')
        });
		return resolve(arr)
	})
}
function musical(URL) {
	return new Promise((resolve, reject) => {
        axios.get('https://musicaldown.com/id', {
            ...
            const url_name = $("#submit-form > div").find("div:nth-child(1) > input:nth-child(1)").attr("name")
...
function multidl(url) {
	return new Promise(async(resolve, reject) =>{
		if(url.includes("tiktok")) {
			musical(url).then((result) => resolve(result)).catch((error) => reject(error))
		}
		else if(url.includes("twitter")) {
			twitter(url).then((result) => resolve(result)).catch((error) => reject(error))
		}
		else if(url.includes("instagram")) {
		ig.fetchPost(url).then((result) => resolve(result)).catch((error) => reject(error))
		}
		else if(url.includes("facebook")) {
			facebook(url).then((result) => resolve(result)).catch((error) => reject(error))
			}
	})
}

module.exports = {openai, randomanime, updateanime, loli, musical, twitter, komikusearch, komikdetail, facebook, tiktokdl,multidl}
